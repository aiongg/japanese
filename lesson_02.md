# Lesson 2: Navigating Train Stations and Airports

こんにちは (*konnichiha*), Edward! Welcome to Lesson 2. Today, we’ll build on your skills by focusing on practical Japanese for train stations (*えき*) and airports (*くうこう*). These are key scenarios for your vacation, covering buying tickets, asking about platforms and facilities, validating passes, checking in, and handling immigration/customs. Let’s dive in with dialogues and examples to get you comfortable with the language.

---

## 駅 (*えき*) での会話 (*かいわ*) - Train Station Conversations

Let’s start with useful phrases at a train station (*えき*). Below is a dialogue followed by additional examples.

### 対話 (*たいわ*) - Dialogue:

**あなた:**  
こんにちは、東京までの切符をください。  
*(Hello, please give me a ticket to Tokyo.)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| こんにちは | - | Hello         |
| 東京       | とうきょう | Tokyo         |
| まで       | -       | Until/to      |
| 切符       | きっぷ     | Ticket        |
| を         | -         | Object marker |
| ください   | -   | Please (give me) |

**駅員 (*えきいん*) - Station Staff:**  
はい、片道ですか、往復ですか？  
*(Yes, one-way or round-trip?)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| はい       | -       | Yes           |
| 片道      | かたみち   | One-way       |
| です       | -       | Is (copula)   |
| か         | -         | Question marker |
| 往復      | おうふく   | Round-trip    |

**あなた:**  
片道でお願いします。いくらですか？  
*(One-way, please. How much is it?)*

| Vocabulary | Kana           | English Gloss |
|------------|----------------|---------------|
| 片道      | かたみち       | One-way       |
| で         | -             | By means of (particle) |
| お願いします | おねがいします | Please (do)   |
| いくら     | -         | How much      |
| です       | -           | Is (copula)   |
| か         | -             | Question marker |

**駅員 (*えきいん*):**  
1500円です。袋にお入れしますか？  
*(It’s 1500 yen. Would you like it in a bag?)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 1500       | せんごひゃく | 1500          |
| 円         | えん       | Yen           |
| です       | -       | Is (copula)   |
| 袋         | ふくろ     | Bag           |
| に         | -         | Particle (location) |
| お入れします | - | Put in (humble) |
| か         | -         | Question marker |

**あなた:**  
いいえ、大丈夫です。ホームはどこですか？  
*(No, it’s fine. Where is the platform?)*

| Vocabulary | Kana           | English Gloss |
|------------|----------------|---------------|
| いいえ     | -         | No            |
| 大丈夫     | だいじょうぶ   | Fine/OK       |
| です       | -           | Is (copula)   |
| ホーム     | -         | Platform      |
| は         | -             | Topic marker  |
| どこ       | -           | Where         |
| です       | -           | Is (copula)   |
| か         | -             | Question marker |

**駅員 (*えきいん*):**  
2番ホームです。あそこにあります。  
*(It’s platform 2. It’s over there.)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 2          | に         | Two           |
| 番         | ばん       | Number (counter) |
| ホーム     | -     | Platform      |
| です       | -       | Is (copula)   |
| あそこ     | -     | Over there    |
| に         | -         | Particle (location) |
| あります   | -   | Exists (for inanimate objects) |

**あなた:**  
トイレはどこですか？  
*(Where is the bathroom?)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| トイレ     | -     | Bathroom      |
| は         | -         | Topic marker  |
| どこ       | -       | Where         |
| です       | -       | Is (copula)   |
| か         | -         | Question marker |

**駅員 (*えきいん*):**  
右にあります。  
*(It’s to the right.)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 右         | みぎ       | Right         |
| に         | -         | Particle (location) |
| あります   | -   | Exists (for inanimate objects) |

### 追加 (*ついか*) の例文 (*れいぶん*) - Additional Examples:

- 大阪までの切符を2枚ください。  
*(Please give me two tickets to Osaka.)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 大阪       | おおさか   | Osaka         |
| まで       | -       | Until/to      |
| 切符       | きっぷ     | Ticket        |
| を         | -         | Object marker |
| 2          | に         | Two           |
| 枚         | まい       | Counter for flat objects (e.g., tickets) |
| ください   | -   | Please (give me) |

- このパスは有効ですか？  
*(Is this pass valid?)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| この       | -       | This          |
| パス       | -       | Pass          |
| は         | -         | Topic marker  |
| 有効       | ゆうこう   | Valid         |
| です       | -       | Is (copula)   |
| か         | -         | Question marker |

- 京都行きの電車は何時ですか？  
*(What time is the train to Kyoto?)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 京都       | きょうと   | Kyoto         |
| 行きの     | いきの     | Bound for     |
| 電車       | でんしゃ   | Train         |
| は         | -         | Topic marker  |
| 何時       | なんじ     | What time     |
| です       | -       | Is (copula)   |
| か         | -         | Question marker |

- 出口はあそこですか？  
*(Is the exit over there?)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 出口       | でぐち     | Exit          |
| は         | -         | Topic marker  |
| あそこ     | -     | Over there    |
| です       | -       | Is (copula)   |
| か         | -         | Question marker |

- 新幹線のホームは何番ですか？  
*(What platform is the Shinkansen?)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 新幹線     | しんかんせん | Shinkansen (bullet train) |
| の         | -         | Possessive particle (of) |
| ホーム     | -     | Platform      |
| は         | -         | Topic marker  |
| 何         | なん       | What          |
| 番         | ばん       | Number (counter) |
| です       | -       | Is (copula)   |
| か         | -         | Question marker |

- 荷物を預けたいです。どこですか？  
*(I want to store my luggage. Where is it?)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 荷物       | にもつ     | Luggage       |
| を         | -         | Object marker |
| 預けたい   | あずけたい | Want to store |
| です       | -       | Is (copula)   |
| どこ       | -       | Where         |
| です       | -       | Is (copula)   |
| か         | -         | Question marker |

- この切符は新幹線で使えますか？  
*(Can this ticket be used for the Shinkansen?)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| この       | -       | This          |
| 切符       | きっぷ     | Ticket        |
| は         | -         | Topic marker  |
| 新幹線     | しんかんせん | Shinkansen    |
| で         | -         | By means of (particle) |
| 使えます   | つかえます | Can use       |
| か         | -         | Question marker |

- 次の電車は何時に来ますか？  
*(When does the next train come?)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 次         | つぎ       | Next          |
| の         | -         | Possessive particle (of) |
| 電車       | でんしゃ   | Train         |
| は         | -         | Topic marker  |
| 何時       | なんじ     | What time     |
| に         | -         | Particle (time) |
| 来ます     | きます     | Comes         |
| か         | -         | Question marker |

---

## 空港 (*くうこう*) での会話 (*かいわ*) - Airport Conversations

Now, let’s move to airport (*くうこう*) scenarios, including check-in, in-flight, and immigration/customs.

### 対話 (*たいわ*) - Dialogue:

**あなた (Check-in):**  
こんにちは、成田行きの便をチェックインします。  
*(Hello, I’d like to check in for the Narita flight.)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| こんにちは | - | Hello         |
| 成田      | なりた     | Narita        |
| 行き      | いき       | Bound for     |
| の         | -         | Possessive particle (of) |
| 便        | びん       | Flight        |
| を         | -         | Object marker |
| チェックイン | - | Check-in      |
| します     | -     | Do            |

**係員 (*かかりいん*) - Staff:**  
パスポートをお預かりします。荷物はお預けになりますか？  
*(I’ll take your passport. Will you check your luggage?)*

| Vocabulary | Kana           | English Gloss |
|------------|----------------|---------------|
| パスポート | -     | Passport      |
| を             | -             | Object marker |
| お預かりします | おあずかりします | Will take (humble) |
| 荷物           | にもつ         | Luggage       |
| は             | -             | Topic marker  |
| お預け         | おあずけ       | Checking (luggage) |
| に             | -             | Particle (indicating result) |
| なります       | -       | Becomes       |
| か             | -             | Question marker |

**あなた:**  
はい、お願いします。搭乗口はどこですか？  
*(Yes, please. Where is the gate?)*

| Vocabulary | Kana           | English Gloss |
|------------|----------------|---------------|
| はい           | -           | Yes           |
| お願いします   | おねがいします | Please (do)   |
| 搭乗口         | とうじょうぐち | Gate          |
| は             | -             | Topic marker  |
| どこ           | -           | Where         |
| です           | -           | Is (copula)   |
| か             | -             | Question marker |

**係員 (*かかりいん*):**  
ゲート12です。搭乗券をどうぞ。  
*(Gate 12. Here’s your boarding pass.)*

| Vocabulary | Kana           | English Gloss |
|------------|----------------|---------------|
| ゲート     | -         | Gate          |
| 12         | じゅうに       | Twelve        |
| です       | -           | Is (copula)   |
| 搭乗券     | とうじょうけん | Boarding pass |
| を         | -             | Object marker |
| どうぞ     | -         | Please (offering) |

**あなた (In-flight):**  
水をもらえますか？  
*(Can I have some water?)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 水         | みず       | Water         |
| を         | -         | Object marker |
| もらえます | - | Can receive   |
| か         | -         | Question marker |

**客室乗務員 (*きゃくしつじょうむいん*) - Flight Attendant:**  
はい、お待ちください。  
*(Yes, please wait.)*

| Vocabulary | Kana           | English Gloss |
|------------|----------------|---------------|
| はい           | -           | Yes           |
| お待ちください | おまちください | Please wait (humble) |

**あなた (Immigration):**  
観光で来ました。  
*(I came for sightseeing.)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 観光       | かんこう   | Sightseeing   |
| で         | -         | For (particle) |
| 来ました   | きました   | Came          |

**入国審査官 (*にゅうこくしんさかん*) - Immigration Officer:**  
何日滞在しますか？  
*(How many days will you stay?)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 何日       | なんにち   | How many days |
| 滞在       | たいざい   | Stay          |
| します     | -     | Do            |
| か         | -         | Question marker |

**あなた:**  
7日です。  
*(7 days.)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 7          | しち       | Seven         |
| 日         | にち       | Day (counter) |
| です       | -       | Is (copula)   |

### 追加 (*ついか*) の例文 (*れいぶん*) - Additional Examples:

- 窓側の席をお願いします。  
*(Please give me a window seat.)*

| Vocabulary | Kana           | English Gloss |
|------------|----------------|---------------|
| 窓側       | まどがわ       | Window side   |
| の         | -             | Possessive particle (of) |
| 席         | せき           | Seat          |
| を         | -             | Object marker |
| お願いします | おねがいします | Please (do)   |

- この便は遅れていますか？  
*(Is this flight delayed?)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| この       | -       | This          |
| 便         | びん       | Flight        |
| は         | -         | Topic marker  |
| 遅れています | おくれています | Is delayed    |
| か         | -         | Question marker |

- 税関はどこですか？  
*(Where is customs?)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 税関       | ぜいかん   | Customs       |
| は         | -         | Topic marker  |
| どこ       | -       | Where         |
| です       | -       | Is (copula)   |
| か         | -         | Question marker |

- 機内持ち込みできますか？  
*(Can I bring it on board?)*

| Vocabulary | Kana           | English Gloss |
|------------|----------------|---------------|
| 機内       | きない         | On board      |
| 持ち込み   | もちこみ       | Bringing in   |
| できます   | -       | Can do        |
| か         | -             | Question marker |

- 毛布をください。  
*(Please give me a blanket.)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 毛布       | もうふ     | Blanket       |
| を         | -         | Object marker |
| ください   | -   | Please (give me) |

- 家族と一緒に旅行します。  
*(I’m traveling with my family.)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 家族       | かぞく     | Family        |
| と         | -         | With          |
| 一緒       | いっしょ   | Together      |
| に         | -         | Particle (indicating companionship) |
| 旅行       | りょこう   | Travel        |
| します     | -     | Do            |

- 日本に2週間滞在します。  
*(I’ll stay in Japan for 2 weeks.)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 日本       | にほん     | Japan         |
| に         | -         | Particle (location) |
| 2          | に         | Two           |
| 週間       | しゅうかん | Week (counter) |
| 滞在       | たいざい   | Stay          |
| します     | -     | Do            |

- 到着は何時ですか？  
*(What time is arrival?)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 到着       | とうちゃく | Arrival       |
| は         | -         | Topic marker  |
| 何時       | なんじ     | What time     |
| です       | -       | Is (copula)   |
| か         | -         | Question marker |

---

### 文法 (*ぶんぽう*) のポイント (*ポイント*) - Grammar Points

1. **～まで - Destination/Extent:**  
   - Indicates a destination or limit (e.g., "to Tokyo").  
   - Example: 「東京まで切符をください。」 *(Please give me a ticket to Tokyo.)*  
   - Additional Examples:  

- 大阪まで行きます。  
  *(I’ll go to Osaka.)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 大阪       | おおさか   | Osaka         |
| まで       | -       | Until/to      |
| 行きます   | いきます   | Go            |

- 駅まで歩きます。  
  *(I’ll walk to the station.)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 駅         | えき       | Station       |
| まで       | -       | Until/to      |
| 歩きます   | あるきます | Walk          |

2. **～たいです - Desire:**  
   - Expresses wanting to do something (e.g., "I want to store").  
   - Example: 「荷物を預けたいです。」 *(I want to store my luggage.)*  
   - Additional Examples:  

- 日本に行きたいです。  
  *(I want to go to Japan.)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 日本       | にほん     | Japan         |
| に         | -         | Particle (destination) |
| 行きたい   | いきたい   | Want to go    |
| です       | -       | Is (copula)   |

- 水を飲みたいです。  
  *(I want to drink water.)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| 水         | みず       | Water         |
| を         | -         | Object marker |
| 飲みたい   | のみたい   | Want to drink |
| です       | -       | Is (copula)   |

3. **～できますか - Ability/Permission:**  
   - Asks if something can be done (e.g., "Can I use?").  
   - Example: 「切符は新幹線で使えますか？」 *(Can this ticket be used for the Shinkansen?)*  
   - Additional Examples:  

- ここで写真を撮れますか？  
  *(Can I take a photo here?)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| ここ       | -       | Here          |
| で         | -         | At (particle) |
| 写真       | しゃしん   | Photo         |
| を         | -         | Object marker |
| 撮れます   | とれます   | Can take      |
| か         | -         | Question marker |

- このパスを使えますか？  
  *(Can I use this pass?)*

| Vocabulary | Kana       | English Gloss |
|------------|------------|---------------|
| この       | -       | This          |
| パス       | -       | Pass          |
| を         | -         | Object marker |
| 使えます   | つかえます | Can use       |
| か         | -         | Question marker |

---

That’s it for Lesson 2! Next time, we’ll explore restaurant and shopping scenarios. Feel free to ask any questions or request clarification. がんばってください (*ganbatte kudasai*)!

---

## 語彙 (*ごい*) リスト (*リスト*) - Vocabulary Glossary

| Vocabulary       | Kana           | English Gloss         | Romanization        |
|------------------|----------------|-----------------------|---------------------|
| こんにちは       | -     | Hello                 | konnichiha          |
| 東京             | とうきょう     | Tokyo                 | tōkyō               |
| まで             | -           | Until/to              | made                |
| 切符             | きっぷ         | Ticket                | kippu               |
| を               | -             | Object marker         | o                   |
| ください         | -       | Please (give me)      | kudasai             |
| はい             | -           | Yes                   | hai                 |
| 片道            | かたみち       | One-way               | katamichi           |
| です             | -           | Is (copula)           | desu                |
| か               | -             | Question marker       | ka                  |
| 往復            | おうふく       | Round-trip            | ōfuku               |
| で               | -             | By means of (particle) | de                 |
| お願いします     | おねがいします | Please (do)           | onegaishimasu       |
| いくら           | -         | How much              | ikura               |
| 1500             | せんごひゃく   | 1500                  | sengohyaku          |
| 円               | えん           | Yen                   | en                  |
| 袋               | ふくろ         | Bag                   | fukuro              |
| に               | -             | Particle (various uses) | ni                |
| お入れします     | おいれします   | Put in (humble)       | oire shimasu        |
| いいえ           | -         | No                    | iie                 |
| 大丈夫           | だいじょうぶ   | Fine/OK               | daijōbu             |
| ホーム           | -         | Platform              | hōmu                |
| は               | -             | Topic marker          | ha                  |
| どこ             | -           | Where                 | doko                |
| 2                | に             | Two                   | ni                  |
| 番               | ばん           | Number (counter)      | ban                 |
| あそこ           | -         | Over there            | asoko               |
| あります         | -       | Exists (for inanimate objects) | arimasu    |
| トイレ           | -         | Bathroom              | toire               |
| 右               | みぎ           | Right                 | migi                |
| 大阪             | おおさか       | Osaka                 | ōsaka               |
| 枚               | まい           | Counter for flat objects | mai              |
| この             | -           | This                  | kono                |
| パス             | -           | Pass                  | pasu                |
| 有効             | ゆうこう       | Valid                 | yūkō                |
| 京都             | きょうと       | Kyoto                 | kyōto               |
| 行きの           | いきの         | Bound for             | iki no              |
| 電車             | でんしゃ       | Train                 | densha              |
| 何時             | なんじ         | What time             | nanji               |
| 出口             | でぐち         | Exit                  | deguchi             |
| 新幹線           | しんかんせん   | Shinkansen (bullet train) | shinkansen    |
| 何               | なん           | What                  | nan                 |
| 荷物             | にもつ         | Luggage               | nimotsu             |
| 預けたい         | あずけたい     | Want to store         | azuketai            |
| 使えます         | つかえます     | Can use               | tsukaemasu          |
| 次               | つぎ           | Next                  | tsugi               |
| 来ます           | きます         | Comes                 | kimasu              |
| 成田            | なりた         | Narita                | narita              |
| 行き            | いき           | Bound for             | iki                 |
| 便              | びん           | Flight                | bin                 |
| チェックイン     | -   | Check-in              | chekkuin            |
| します           | -         | Do                    | shimasu             |
| パスポート       | -     | Passport              | pasupōto            |
| お預かりします   | おあずかりします | Will take (humble)  | oazukari shimasu    |
| お預け           | おあずけ       | Checking (luggage)    | oazuke              |
| なります         | -       | Becomes               | narimasu            |
| 搭乗口           | とうじょうぐち | Gate                  | tōjōguchi           |
| ゲート           | -         | Gate                  | gēto                |
| 12               | じゅうに       | Twelve                | jūni                |
| 搭乗券           | とうじょうけん | Boarding pass         | tōjōken             |
| どうぞ           | -         | Please (offering)     | dōzo                |
| 水               | みず           | Water                 | mizu                |
| もらえます       | -     | Can receive           | moraemasu           |
| お待ちください   | おまちください | Please wait (humble)  | omachi kudasai      |
| 観光             | かんこう       | Sightseeing           | kankō               |
| 来ました         | きました       | Came                  | kimashita           |
| 何日             | なんにち       | How many days         | nannichi            |
| 滞在             | たいざい       | Stay                  | taizai              |
| 7                | しち           | Seven                 | shichi              |
| 日               | にち           | Day (counter)         | nichi               |
| 窓側             | まどがわ       | Window side           | madogawa            |
| 席               | せき           | Seat                  | seki                |
| 遅れています     | おくれています | Is delayed            | okureteimasu        |
| 税関             | ぜいかん       | Customs               | zeikan              |
| 機内             | きない         | On board              | kinai               |
| 持ち込み         | もちこみ       | Bringing in           | mochikomi           |
| できます         | -       | Can do                | dekimasu            |
| 毛布             | もうふ         | Blanket               | mōfu                |
| 家族             | かぞく         | Family                | kazoku              |
| 一緒             | いっしょ       | Together              | issho               |
| 旅行             | りょこう       | Travel                | ryokō               |
| 日本             | にほん         | Japan                 | nihon               |
| 2                | に             | Two                   | ni                  |
| 週間             | しゅうかん     | Week (counter)        | shūkan              |
| 到着             | とうちゃく     | Arrival               | tōchaku             |
| 行きます         | いきます       | Go                    | ikimasu             |
| 歩きます         | あるきます     | Walk                  | arukimasu           |
| 行きたい         | いきたい       | Want to go            | ikitai              |
| 飲みたい         | のみたい       | Want to drink         | nomitai             |
| 写真             | しゃしん       | Photo                 | shashin             |
| 撮れます         | とれます       | Can take              | toremasu            |
